...
pub fn get_log_quotient_degree<F, A>(rap: &A, num_public_values: usize) -> usize
where
    F: Field,
    A: Rap<SymbolicAirBuilder<F>>,
{
    // We pad to at least degree 2, since a quotient argument doesn't make sense with smaller degrees.
    let constraint_degree = get_max_constraint_degree(rap, num_public_values).max(2);
...
pub fn get_max_constraint_degree<F, A>(rap: &A, num_public_values: usize) -> usize
where
    F: Field,
    A: Rap<SymbolicAirBuilder<F>>,
{
    get_symbolic_constraints(rap, num_public_values)
        .iter()
    ...
    A: Rap<SymbolicAirBuilder<F>>,
{
    let mut builder = SymbolicAirBuilder::new(
        rap.preprocessed_width(),
        rap.width(),
        rap.permutation_width(),
        num_public_values,
    );
    rap.eval_all(&mut builder);
    builder.constraints()
}
